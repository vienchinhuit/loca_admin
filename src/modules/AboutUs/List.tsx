...
import { Item as Type, ListConfig } from "./type";
...
export default function Item() {
  const routePage = `${path.ROUTE_ADMIN}${path.ROUTE_ABOUT_US}`;
  ...
  const onFinishCreate = (values: Type) => {
  ...
  const onFinishEdit = (values: Type) => {
    const formData = new FormData();
    if (values && typeof values === "object") {
      (Object.keys(values) as (keyof Type)[]).forEach((key) => {
        let value = values[key];
